  ...
  try {
    res.json({
      ...
      data: {
        ...
        sounds: db.get("sounds").value(),
        lamps: db.get("lamps").value(),
        screens: db.get("screens").value()
      }
    })
  } catch (error) {
    res.json({
...
function getSubOrder(id) {
  ...
  subOrder.orderInfo = subOrder.orderInfo.map(order => {
    order.detail = order.detail.map(item => {
      let name = item.type == 1 ? "screens" : item.type == 2 ? "lamps" : "sounds"
      return db.get(name).find({id: item.id}).value();
    })
    return order;
  })
  return subOrder;
